...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_HVAC_MODES, default=','.join(DEFAULT_HVAC_MODES)): mode_validator(DEFAULT_HVAC_MODES),
    ...
    vol.Optional(CONF_FAN_MODES, default=','.join(DEFAULT_FAN_MODES)): mode_validator(DEFAULT_FAN_MODES),
    vol.Optional(CONF_SWING_MODES, default=','.join(DEFAULT_SWING_MODES)): mode_validator(DEFAULT_SWING_MODES),
    vol.Optional(CONF_SWING_HORIZONTAL_MODES, default=','.join(DEFAULT_SWING_HORIZONTAL_MODES)): mode_validator(DEFAULT_SWING_HORIZONTAL_MODES),
...
async def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    hvac_modes = [getattr(HVACMode, mode.strip().upper()) for mode in string_to_list(config.get(CONF_HVAC_MODES)).split(",") if mode.strip()]
...
class GreeClimate(ClimateEntity):
    ...
    def __init__(self, hass, name, ip_addr, port, mac_addr, timeout, target_temp_step, temp_sensor_entity_id, lights_entity_id, xfan_entity_id, health_entity_id, powersave_entity_id, sleep_entity_id, eightdegheat_entity_id, air_entity_id, target_temp_entity_id, anti_direct_blow_entity_id, hvac_modes, fan_modes, swing_modes, swing_horizontal_modes, auto_xfan_entity_id, auto_light_entity_id, light_sensor_entity_id, encryption_version, disable_available_check, max_online_attempts, encryption_key=None, uid=None, beeper_entity_id=None, temp_sensor_offset=None):
